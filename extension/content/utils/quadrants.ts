...
export function createQuadrantOverlays() {
  console.log("createQuadrantOverlays called", { args: [] });
  // Remove any existing overlays first to avoid duplicates
  document
    .querySelectorAll(".quadrant-overlay")
  ...
  hideAllQuadrants();
  console.log("createQuadrantOverlays finished");
}

export function hideAllQuadrants() {
  console.log("hideAllQuadrants called", { args: [] });
  ...
  console.log("hideAllQuadrants finished");
}

// Helper function to determine which corner the element is positioned at
export function getCurrentCorner(element: HTMLElement) {
  console.log("getCurrentCorner called", { element });
  ...
  console.log("getCurrentCorner finished");
  return "top-right";
}


export function makeDivDraggable(element: HTMLElement) {
  console.log("makeDivDraggable called", { element });
  ...
  function startDrag(e: MouseEvent | TouchEvent) {
    console.log("startDrag called", { type: e.type });
    e.preventDefault();
    ...
    if (e.type === "touchstart") {
      ...
      initialX = touchEvent.touches[0].clientX;
      initialY = touchEvent.touches[0].clientY;
    } else {
    ...
    document.querySelectorAll(".quadrant-overlay").forEach((overlay) => {
      ...
      (overlay as HTMLElement).style.opacity = "0"; // Still transparent
    });
    ...
    console.log("startDrag finished");
  }

  function drag(e: MouseEvent | TouchEvent) {
    console.log("drag called", { type: e.type });
    ...
    if (e.type === "touchmove") {
      const touchEvent = e as TouchEvent;
      currentMouseX = touchEvent.touches[0].clientX;
      currentMouseY = touchEvent.touches[0].clientY;
    } else {
      const mouseEvent = e as MouseEvent;
  ...
  function stopDrag(): void {
    console.log("stopDrag called", { args: [] });
    if (!isDragging) return;
    ...
    offset.x = 0;
    offset.y = 0;
    console.log("stopDrag finished");
  }

  function snapToClosestCorner(): void {
    console.log("snapToClosestCorner called", { args: [] });
    const rect = element.getBoundingClientRect();
    const windowWidth = window.innerWidth;
    const windowHeight = window.innerHeight;
    ...
    console.log("snapToClosestCorner finished");
  }
}

export function positionDivAtCorner(corner: string): void {
  console.log("positionDivAtCorner called", { corner });
  const draggableDiv = document.getElementById(
    "your-mom-draggable"
  ) as HTMLElement | null;
  if (!draggableDiv) {
    console.error("positionDivAtCorner error: Element not found");
    return; // Exit if element not found
  }

  const offset: number = 10; // 10px from the edge
  ...
  console.log("positionDivAtCorner finished");
}
